...
To generate the graph my first attempt was to connect points that are close enough. This created a messy result which a seperate de-messifying algorithm needed to clean up. This still resulted in mostly messy graphs, which i didn't notice at first because of survivorship bias. I only found out when i started profiling and realized that the testing stage required way more attempts than it was supposed to, even with generous requirements.
...
There are still some problems with this graph though. First of all, a proper voronoi graph has lines on the outside that infinitely extend. This algorithm doesn't generate those lines, because there are no triangles there that border each other, so i had to fix that. Then, i didn't want the dungeon to end up with infinite lines, so i needed a boundary around the whole dungeon. I did this by calculating a convex hull from the initial points and then offsetting that hull to create a boundary around the dungeon. I then used this boundary to limit the lines to the intersection point between the infinite line and the border. This creates a nice end of the dungeon.

The circles are generated by three points, but if those points are close to a line, the resulting circle is very large. The centerpoint of that circle can even end up outside the convex hull creating a weird looking dungeon. To solve this i decided to move the middle point along the axis of the infinite line away from the boundary, by interpolating between the center of the circle and the center of the triangle.

Another problem that came up was that some edges were too short to fit a door. Simply not generating the door would lead to a discrepancy between the pathfinding algorithm and the final result. I solved this by deleting the corresponding connection in the delauney graph, so the pathfinding algorithm doesn't assume that there is a door, and the door never get's generated.
